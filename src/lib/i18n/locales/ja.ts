...
const translations = {
  ...commonTranslations,
  nav: {
  home: 'ホーム', blog: 'ブログ', pricing: '価格', about: '会社概要', login: 'ログイン', register: '登録', dashboard: 'ダッシュボード', admin: '管理パネル', logout: 'ログアウト', credits: 'クレジット', download: 'アプリをダウンロード', light: 'ライト', dark: 'ダーク',
  },
  footer: {
...
,
{
    about: {
      ...
      features: {
        ...
        aiIntelligence: {
          title: AI驅動インテリジェンス,
          desc: 高度なAIアルゴリズムが動画コンテンツを自動的に分析し、最も魅力的な瞬間を特定、クリエイターの手動編集を数時間節約します。
        },
        fastProcessing: {
          title: 超高速処理,
          desc: 長編動画を数分で処理。我们的最適化されたパイプラインがハイライトを即座に配信します。
        },
        multiPlatform: {
          title: マルチプラットフォームサポート,
          desc: YouTube、BiliBiliからインポートまたはローカルファイルをアップロード。TikTok、YouTube Shorts、Instagramなどの最適化されたクリップをエクスポート。
        },
        privacyFirst: {
          title: プライバシー優先,
          desc: あなたのコンテンツはあなただけのもの。元の動画を保存することは決してなく、すべて安全に処理されます。
        }
      },
      values: {
        title: 私たちのコアバリュー,
        ...
        userCentric: {
          title: ユーザー中心のイノベーション,
          desc: 世界中のコンテンツクリエイターの実際の問題を解決するツールを構築します。
        },
        continuous: {
          title: 継続的な改善,
          desc: ユーザーフィードバックに基づく定期的なアップデートで体験を向上。
        },
        community: {
          title: グローバルコミュニティ,
          desc: 世界中の6大陸で32の言語をサポートするクリエイター支援。
        }
      },
      geo: {
