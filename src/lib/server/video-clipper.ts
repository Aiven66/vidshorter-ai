  ...
  for (const instance of instancesToTry) {
    try {
      ...
      const data = await res.json() as {
        ...
        audioStreams?: Array<{ url: string; quality?: string; mimeType?: string; bitrate?: number }>;
      ...
      // Best combined (muxed) stream — usually limited to 360p/720p
      const bestCombined =
        combined
      ...
      // Best video-only stream — can reach 1080p/4K (requires separate audio)
      const bestVideoOnly =
        videoOnly
          .map(s => ({ s, q: parseQuality(s.quality) }))
          .filter(item => item.q > 0 && item.q <= YOUTUBE_MAX_HEIGHT)
      ...
      let stream: { url: string; quality?: string } | undefined;
      let audioUrl: string | undefined;

      // Prefer HD video-only + separate audio if it offers higher quality than combined
      if (bestVideoOnly && videoOnlyQ > combinedQ && videoOnlyQ >= 720) {
        const bestAudio = (data.audioStreams || [])
          .filter(a => a.url && a.mimeType?.includes('mp4'))
          .sort((a, b) => (b.bitrate || parseQuality(b.quality)) - (a.bitrate || parseQuality(a.quality)))[0];
        if (bestAudio?.url) {
      ...
      // Fall back to combined stream (or first available)
      if (!stream) {
    ...
    try {
      ...
      const data = await res.json() as {
        ...
        adaptiveFormats?: Array<{
          url?: string; type?: string; qualityLabel?: string; bitrate?: string;
          audioSampleRate?: number; fps?: number;
        }>;
      };
      const parseQuality = (value?: string) => {
        const m = value?.match(/(\d{3,4})/);
      ...
      // Combined (muxed) streams — usually limited to 360p/720p
      ...
      // Adaptive formats — separate video-only (up to 1080p/4K) and audio-only
      const adaptive = (data.adaptiveFormats || []).filter(f => f.url);
      const videoOnlyAdaptive = adaptive.filter(f =>
        f.type?.startsWith('video/mp4') && !f.audioSampleRate
      );
      const audioOnlyAdaptive = adaptive.filter(f =>
        f.type?.startsWith('audio/mp4') || f.type?.includes('audio/')
      );
      ...
      let stream: { url: string; qualityLabel?: string } | undefined;
      let audioUrl: string | undefined;

      // Prefer HD video-only + separate audio if higher quality than combined
      if (bestVideoOnly && videoOnlyQ > combinedQ && videoOnlyQ >= 720) {
        const bestAudio = audioOnlyAdaptive
...
async function analyzeYouTubeViaPipedAndTranscript(videoUrl: string): Promise<VideoAnalysisResult> {
  ...
  // Fallback: scrape YouTube watch page for lengthSeconds when all proxies failed.
  // This is the most reliable keyless method — the watch page is public HTML.
  // Fixes the issue where duration stays at 300 (5 min) when Invidious/Piped/CFWorker
  // all timeout on Vercel, causing only 5 clips instead of 10 for longer videos.
  if (duration <= 300) {
...
async function createLocalClip(params: {
  ...
  const isWorkerStream = isRemoteInput && (() => {
    try {
      ...
      // Internal Edge proxy URLs (/api/yt-stream?proxy=1) — support Range, use fast seek
      if (internalBaseUrl && u.toString().startsWith(internalBaseUrl) && u.pathname.includes('/yt-stream') && u.searchParams.get('proxy') === '1') {
        return true;
      }
      return false;
