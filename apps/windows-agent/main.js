...
// Windows 单实例锁：deep link 点击会启动新进程，必须阻止并转发给主实例
const gotTheLock = app.requestSingleInstanceLock();
if (!gotTheLock) {
  // 第二实例：检查 argv 是否含 deep link URL，转发给主实例后退出
  const lastArg = process.argv[process.argv.length - 1];
  if (lastArg && lastArg.startsWith('clipop://')) {
    // 主实例会通过 second-instance 事件接收到
  }
  app.quit();
  process.exit(0);
}
...
// ==================== LIFECYCLE ====================
// 注：Mac 上使用 app.on('open-url', ...) 接收 deep link
// Windows 上 deep link 通过 second-instance 事件 + 启动参数 argv 接收
app.on('second-instance', (event, commandLine, workingDirectory) => {
  appendLog(`[App] second-instance event: ${JSON.stringify(commandLine)}`);
  // 从 commandLine 中查找 clipop:// URL
  let deepLinkUrl = '';
  for (const arg of commandLine) {
    if (typeof arg === 'string' && arg.startsWith('clipop://')) {
      deepLinkUrl = arg;
      break;
    }
  }
  if (deepLinkUrl) {
    appendLog(`[App] second-instance deep link: ${deepLinkUrl}`);
    handleDeepLink(deepLinkUrl).catch((err) => {
      appendLog(`[App] second-instance handleDeepLink ERROR: ${err}`);
    });
  } else {
    // 单纯唤起应用：聚焦主窗口
    if (webWindow && !webWindow.isDestroyed()) {
...
app.on('ready', async () => {
  appendLog(`[App] Ready (v${APP_VERSION}) platform=${process.platform}`);
  detectLocale();
  appendLog(`[App] Detected locale: ${currentLocale}`);

  // 日志路径：Windows 没有 app.getPath('logs')，使用 userData/logs
  let logDir;
  if (process.platform === 'win32') {
    logDir = path.join(app.getPath('userData'), 'logs', 'ClipopAgent');
  } else {
    logDir = path.join(app.getPath('logs'), 'ClipopAgent');
  }
  ...
  // Windows: 检查启动参数中是否有 deep link URL
  // 当用户点击 clipop:// 链接且应用未运行时，Windows 会以 URL 作为 argv 启动应用
  if (process.platform === 'win32') {
    for (const arg of process.argv) {
      if (typeof arg === 'string' && arg.startsWith('clipop://')) {
        appendLog(`[App] Found deep link in argv: ${arg}`);
        // 延迟处理，等 webWindow 准备好
        setTimeout(() => {
          handleDeepLink(arg).catch((err) => {
            appendLog(`[App] argv deep link handle ERROR: ${err}`);
          });
        }, 2000);
        break;
      }
