...
function normalizeHighlights(input, duration) {
  ...
  const normalized = input.filter((item) => item && typeof item === "object").map((item, index) => {
    ...
    const summary = typeof item.summary === "string" && item.summary.trim() ? item.summary.trim().slice(0, 160) : "Automatically selected highlight from the source video.";
...
async function getYouTubeStreamUrlViaYtDlp(videoId) {
  ...
  const args = [
    ...
    "-f",
    `best[ext=mp4][height<=${maxH}]/best[height<=${maxH}]/best`,
    "-g",
    videoUrl
  ];
...
async function createLocalClip(params) {
  ...
  const seekArgs = !isRemoteInput || isWorkerStream ? ["-ss", String(params.startTime), "-i", params.inputPath, "-t", String(duration)] : ["-i", params.inputPath, "-ss", String(params.startTime), "-t", String(duration)];
  const args = [
    "-y",
    ...ffmpegInputHeaders,
    ...httpInputFlags,
...
async function processJob(serverUrl, headers, job) {
  ...
  const highlights = analysis.highlights.slice(0, Math.max(1, Math.min(10, job.desiredClipCount || 3)));
  await report(serverUrl, headers, {
    jobId: job.id,
    status: "processing",
...
async function main() {
  ...
  if (!process.env.INLINE_CLIPS) process.env.INLINE_CLIPS = "1";
  for (; ; ) {
    try {
